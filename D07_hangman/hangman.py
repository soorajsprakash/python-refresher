...
while lives > 0:
    if '_' in display:
      ...
      clear()
      for eachPos in range(len(chosen_word)):
          if guess == chosen_word[eachPos]:
              display[eachPos] = chosen_word[eachPos]
      if guess in chosen_word:
        if guess in display:
          print(f"You have already entered {guess} before, please try again with diff guess.")
      if guess not in chosen_word:
        print(f"You entered {guess}, but its not in the word. You lose a life")
        lives -= 1
        if lives == 0:
            print('YOU LOSE')
      print(display)
